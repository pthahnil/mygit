...
public class ParticleRenderer {
	...
	protected ParticleRenderer(Loader loader, Matrix4f projectionMatrix) {
		this.loader = loader;
		this.vbo = loader.createEmptyVbos(INSTANCE_DATA_LENGTH * MAX_INSTANCES);
	...
	protected void render(Map<ParticleTexture, List<Particle>> particles, Camera camera) {
		...
		for (ParticleTexture texture : particles.keySet()) {
			...
			for (Particle particle : list) {
				updateModelViewMaxrit(particle.getPosition(), particle.getRolation(), particle.getScale(), viewMatrix,
						vboData);
				updataTexCoordInfo(particle, vboData);
			}
	...
	private void updataTexCoordInfo(Particle particle, float[] data){
	...
	private void updateModelViewMaxrit(Vector3f position, float rocation, float scale, Matrix4f viewMatrix,
			float[] vboData) {
		...
		Matrix4f.rotate((float) Math.toRadians(scale), new Vector3f(0, 0, 1), modelMatrix, modelMatrix);
